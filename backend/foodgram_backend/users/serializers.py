...
class CustomUserCreateSerializer(UserCreateSerializer):
    ...
    def validate(self, attrs):
        ...
        if User.objects.filter(email=current_email).exists():
            raise serializers.ValidationError({'email': ["Уже есть пользователь с таким email."]})
        for required_attr in required_attrs:
            if attrs.get(required_attr) is None:
                raise serializers.ValidationError({required_attr: ["Это поле обязательно для заполнения."]})
            if attrs.get(required_attr) == '':
                raise serializers.ValidationError({required_attr: ["Это поле не должно быть пустым."]})
